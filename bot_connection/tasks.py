...
def send_msg_to_bot():
    ...
    msgs = SendMessage.objects.filter(status=-1)
    if msgs.exists():
        for msg in msgs:
            obj = SendMessage.objects.get(uuid=msg.uuid)
            obj.status = 1
            obj.save()
            respons = CommandRunner.celery_send_msg(msg.customer.chat_id, msg.message)
            obj.status = respons
            obj.save()
...
#                                 CommandRunner.send_end_of_config_notif(config_mdl.chat_id.chat_id, api[name])
...
# TODO: use model signals to send notif
# @shared_task
